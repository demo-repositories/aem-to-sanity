...
To adopt: register the tabs/accordion component as a plain (non-flatten) container in `aem-component-containers.json` and opt the panel container's resource type into the `cq:panelTitle` authoring hint in `aem-component-hints.json`, then re-run `migrate:schema` + `transform` + `import --discard-drafts`. Pages whose preserved panels would exceed Sanity's hard 20-level attribute-depth limit are repaired automatically: the transform flattens the deepest titled panels until the document fits, keeps the outer tab structure, and lists every sacrificed title in `transform-report.json → depthFlattenedPanels` (plus a console warning naming the page).
