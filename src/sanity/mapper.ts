...
export async function mapDialog(
  ...
  return { fields, unmapped: ctx.unmapped, groups: ctx.groups, renamed: ctx.renamed };
}
...
function dedupeFieldNames(
  ...
  for (const field of fields) {
    ...
    if (candidate !== original) {
      renamed.push({ originalName: original, newName: candidate, title: field.title });
...
async function buildField(
  ...
  switch (kind) {
    case "string":
      return { ...common, type: "string", initialValue: stringAttr(node.value) };
    case "text":
      return {
        ...common,
        type: "text",
    ...
    case "boolean":
      return {
        ...
        type: "boolean",
        initialValue:
          typeof node.value === "boolean" ? node.value : isTruthyAttr(node.value),
      };
    case "date":
      return {
        ...common,
    ...
    case "select":
      return { ...common, type: "string", options: { list: extractSelectItems(node) } };
    case "radio":
      return {
        ...common,
        type: "string",
...
function buildPlaceholder(
  ...
  return {
    name,
    title: stringAttr(node.fieldLabel) ?? stringAttr(node["jcr:title"]) ?? name,
...
function extractSelectItems(node: DialogNode): Array<{ title: string; value: string }> {
