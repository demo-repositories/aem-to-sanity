...
| `AEM_COMPONENT_NAMES_FILE` | optional | `migrate:schema` only. JSON file mapping `sling:resourceType` → explicit Sanity type name (string) or `{ "name", "title", "folder" }`, overriding the `MIGRATION_TYPE_NAMING` strategy per component (`name`/`title`) and the file's subfolder inside the generated schemas dir (`folder`). Default: `./aem-component-names.json`. Missing file → strategy naming only. Set-once-before-first-import hazard for `name` (not for `folder`, which never changes type names), same as `MIGRATION_TYPE_NAMING`. See § 1c-octies. |
...
- A plain string value is shorthand for `{ "name": ... }`. `title` alone is allowed (rename in the Studio without touching the type name), and so is `folder` alone.
...
- **Explicit names win over the strategy** (path- or title-derived) and are claimed first: another component whose derived name collides with an override gets the usual collision fallback (path-derived suffix). Every override and fallback is logged.
