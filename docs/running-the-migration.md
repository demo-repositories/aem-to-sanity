...
| `AEM_PAGE_COMPONENTS_FILE` | optional | JSON file declaring page-shell components and the `cq:template` paths each is authored under (either listed explicitly via `"templates": [...]` or auto-discovered from extracted content via `"discover": true`). Each (resourceType, template) pair becomes one Sanity document type; the page-shell dialog becomes the doc's `pageProperties` field. A per-template `"names"` map pins the emitted `_type` / Studio title (default derives from the template path + `Page` suffix); a `"skipProperties"` array drops selected `jcr:content` properties from `pageProperties` end-to-end (schema field + lifted value). Default: `./aem-page-components.json`. Missing file → no per-template docs (every page falls back to the generic `page` type). See § 1c-septies. |
...
**Skipping page properties.** Add a `skipProperties` array to leave selected `jcr:content` properties behind (tenant bookkeeping, legacy toggles, AEM-only rendering hints):
...
Each listed property is dropped end-to-end: `migrate:schema` omits the matching field from the page-shell's `pageProperties` object (matched by the camelCased dialog field name, so `cq:designPath` matches `cqDesignPath`), and `aem-transform` skips lifting the authored value (matched on the raw `jcr:content` property name, carried via the `page-templates.json` manifest). The doc-level carve-outs (`cq:tags` → `tags`, `cq:featuredimage` → `featuredImage`, `cq:template`) are separate fields and unaffected. Unlike `names`, this is safe to change between runs — rerun `migrate:schema` → `transform` → `import` and the fields appear/disappear without any type rename (already-imported values stay on existing docs until re-imported).
