...
| `AEM_COMPONENT_NAMES_FILE` | optional | `migrate:schema` only. JSON file mapping `sling:resourceType` → explicit Sanity type name (string) or `{ "name", "title" }`, overriding the `MIGRATION_TYPE_NAMING` strategy per component. Default: `./aem-component-names.json`. Missing file → strategy naming only. Set-once-before-first-import hazard, same as `MIGRATION_TYPE_NAMING`. See § 1c-octies. |
...
Consumed by `migrate:schema`. Optional per-component overrides for the emitted Sanity **type name** and/or Studio **title**, keyed by `sling:resourceType` (a leading `/apps/` on the key is accepted). Use it when the `MIGRATION_TYPE_NAMING` strategy produces names that don't match your vocabulary — e.g. AEM titles its `.../content/lists` component "List" and its `.../content/list` component "Page List", so title naming assigns the names backwards from what operators expect.
...
- A plain string value is shorthand for `{ "name": ... }`. `title` alone is allowed (rename in the Studio without touching the type name).
