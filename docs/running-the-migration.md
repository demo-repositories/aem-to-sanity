...
- **`icon`** sets the component's Studio icon. The value is a named export of [`@sanity/icons`](https://icons.sanity.io) (PascalCase, e.g. `"ControlsIcon"`); the generated schema imports it and declares `defineType({ icon: ControlsIcon })`, so the icon shows up in the page-builder insert menu, array item previews, and structure lists. Values that aren't PascalCase identifiers are a hard error at config load; a well-formed name that doesn't exist in `@sanity/icons` surfaces as a Studio typecheck error ("has no exported member"). The consuming Studio needs `@sanity/icons` in its dependencies — scaffolded studios ship it; older studios pick it up via `pnpm -w studio:sync <slug> --fix`. Like `folder`/`file`, **safe to add, change, or drop between runs** — icons never touch type names or ingested content.
