...
3. **Set-once knobs reshape identity.** `MIGRATION_DOC_ID_PREFIX_STRIP`, `MIGRATION_TYPE_NAMING`, `MIGRATION_TYPE_SUFFIX` (in default `type` mode), and `name` overrides in `aem-component-names.json` all feed ingested `_id`/`_type` values. Changing any after the first real import orphans documents. Treat semantic changes to them as breaking.
