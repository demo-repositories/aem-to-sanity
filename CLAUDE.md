...
- **Tenant template change** (anything under `tenants/template/`) → keep the doctor's classification in `packages/aem-to-sanity-cli/src/lib/tenant-template.ts` in sync so `aem-to-sanity doctor` doesn't silently miss the new surface, and rebuild the cli package (its build embeds the template into the published tarball). Details below.
...
- **New file added to `tenants/template/`** → add its basename to either `TEMPLATE_FILES` (template-owned content like `README.md`, `*.example`) or `OPERATOR_FILES` (operator-customized like `aem-*-roots`, `*.json`) in `packages/aem-to-sanity-cli/src/lib/tenant-template.ts` (the doctor/studio-sync implementation lives in that package; `scripts/lib/tenant-template.ts` is a re-export shim adding monorepo path constants). Files in neither set are silently ignored by doctor.
- **The `studio/` subtree** (`tenants/template/studio/`) is the per-tenant Studio template, copied along by `migrate:init` (which renames its workspace to `tenant-<slug>-studio`) and backfilled into existing tenants by `pnpm -w studio:init <slug>`. It is deliberately unclassified in doctor — operator studios diverge freely. Files added inside it need no doctor registration, but keep the workspace-rename step in both init scripts working (`studio/package.json` must exist) and keep `tenants/*/studio` in `pnpm-workspace.yaml`. Existing tenant studios pick up template file additions via `pnpm -w studio:sync <slug> [--fix]` (`packages/aem-to-sanity-cli/src/studio-sync.ts`) — copy-new, add-missing-deps, never overwrite.
- **Migration-critical Studio code does NOT go in the template** — it goes in `packages/aem-to-sanity-studio` (category taxonomy type, AEM widget input components, the `aemSource` ML aspect), which both `apps/studio` and tenant studios import. Template files should be a thin shell (config, re-exports, scripts). Rationale: package code reaches existing operator studios via a toolkit update; template files only reach studios scaffolded after the change (or via an explicit `studio:sync`). If a pipeline stage starts depending on a new type/aspect/component, add it to the package and re-export where needed.
- **New required env var added to `.env.example`** → no code change needed. The doctor parses `.env.example` at runtime and treats every uncommented `KEY=value` line as required. The placeholder regex (`PLACEHOLDER_RE` in `packages/aem-to-sanity-cli/src/lib/tenant-template.ts`) detects leftover values; add a pattern there if you introduce a new placeholder shape (e.g. `<your-thing>`, `xxxx`).
...
- **New conditional env requirement** (e.g. "var X is required only when var Y has a particular value") → add a special-case block in `checkEnv` inside `packages/aem-to-sanity-cli/src/doctor.ts`. The `MIGRATION_DRY_RUN=false` → `SANITY_MEDIA_LIBRARY_ID` check is the existing template.
- **New AEM authentication flow** → extend `AEM_AUTH_FLOWS` in `packages/aem-to-sanity-cli/src/lib/tenant-template.ts` (name + keys + `any-of`/`all-of` mode). The doctor's "no AEM auth configured" error iterates that list.
...
- **New ignored directory** (build output, cache, etc. that init must skip on copy) → add to `IGNORED_NAMES` in `packages/aem-to-sanity-cli/src/lib/tenant-template.ts` AND to the skip set in its `scripts/embed-template.ts` + `create-aem-to-sanity`'s `COPY_SKIP`.
...
`/apps/.../image` would collide with Sanity's built-in `image` type. `resolveSanityTypeNames` (in `packages/aem-to-sanity-schema/src/naming.ts`) applies an `aem` prefix at emission time so the on-disk schema, the content registry, `pageBuilder.of[]`, and ingested document `_type` values all agree — no Studio-side rename, no orphaned content. Don't reintroduce per-import renames in `sanitize.ts`; it's a defense-in-depth pass for hand-authored schemas only. `table`, `row`, and `cell` are also reserved (not Sanity built-ins, but claimed by the native Portable Text table feature — `pt-table.ts` emits toolkit-owned types under those names), so a component like `/apps/.../table` becomes `aemTable`.

Type names derive from the JCR path by default; `MIGRATION_TYPE_NAMING=title` switches to camelCased `jcr:title` (path fallback on missing titles, path-derived suffix on title collisions — `migrate:schema` pre-fetches component nodes to know titles before names resolve). Individual components can pin an explicit name and/or Studio title via the optional tenant file `aem-component-names.json` (keyed by resource type; explicit names claim first, derived collisions take the fallback; reserved built-ins and duplicates are hard errors). Strategy and overrides are set-once-before-first-import knobs: switching either (or renaming a `jcr:title` in AEM) renames emitted types and orphans ingested `_type` values.
...
- **`array-of-blocks`** → Portable Text via `@portabletext/block-tools` + `jsdom` (`packages/aem-to-sanity-content/src/portable-text.ts` — extracted from `transform.ts` so it's unit-testable). Decorators / styles / lists / `link` annotations preserved. HTML `<table>` → native PT table blocks (`table → rows[] → cells[] → value`; `headerRows` from `<thead>` or leading all-`<th>` rows; colspan/rowspan dropped with rows padded rectangular; nested/malformed tables flatten to text — never dropped). `_key`s SHA1-seeded for deterministic diffs.
