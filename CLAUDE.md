...
- **The `studio/` subtree** (`tenants/template/studio/`) is the per-tenant Studio template, copied along by `migrate:init` (which renames its workspace to `tenant-<slug>-studio`) and backfilled into existing tenants by `pnpm -w studio:init <slug>`. It is deliberately unclassified in doctor — operator studios diverge freely. Files added inside it need no doctor registration, but keep the workspace-rename step in both init scripts working (`studio/package.json` must exist) and keep `tenants/*/studio` in `pnpm-workspace.yaml`.
