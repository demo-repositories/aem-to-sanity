...
| `aem-dialog-overrides.json` | Map `sling:resourceType` → `{supplementaryTabs: [{path, insertAfter}]}` and/or `{dialogFile}`. Splices Sling-Resource-Merger-inherited dialog tabs the supertype walk can't see (fetched from the given JCR path), or replaces the whole dialog with a local JSON file. Empty by default. |
