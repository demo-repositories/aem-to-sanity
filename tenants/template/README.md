...
| `aem-component-names.json` | Map `sling:resourceType` → explicit Sanity type name (string) or `{name, title, folder, file}`, overriding the `MIGRATION_TYPE_NAMING` strategy per component (`name`/`title`), the emitted file's subfolder (`folder`), and its exact basename + export const (`file`). Empty by default. |
