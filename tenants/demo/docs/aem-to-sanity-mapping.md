...
| `granite/ui/components/coral/foundation/well` | `container` | Well (static grouping box) → non-collapsible fieldset titled from `jcr:title` or the well's first `heading` item; untitled wells flatten and children hoist up |
...
| `granite/ui/components/coral/foundation/heading` | `hidden` | Coral heading → not a field itself; when it's the first item of a well, its `text` becomes the well's fieldset title |
