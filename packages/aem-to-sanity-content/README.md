...
- `--download-only` (or `MIGRATION_ASSETS_DOWNLOAD_ONLY=true`) — run phase 1 (download from AEM) for real, then stop: phase 0's ML dedup lookup, phase 2 upload, phase 3 link, and phase 4 rewrite are all skipped unconditionally, independent of `MIGRATION_DRY_RUN`. Needs no Sanity Media Library / token / project env vars at all. For operators without ML access, or who plan to hand the cached binaries in `output/cache/assets/` + `manifest.json` off to a different asset platform (e.g. Bynder) instead of the Sanity Media Library. Mutually exclusive with `--link-only` and `--upload-only`. See § *`--download-only` mode*.
...
- `MIGRATION_ASSET_BACKEND=bynder` — resolve assets against a Bynder portal instead of the Sanity Media Library: phases 1–3 are skipped, phase 0 resolves each DAM path via a metaproperty holding the legacy AEM path, and phase 4 rewrites fields to `bynder.asset` objects. Needs `BYNDER_BASE_URL` + `BYNDER_TOKEN` + `BYNDER_AEM_PATH_PROPERTY`. Rejects `--upload-only` / `--placeholders`; `--link-only` is implied; `--download-only` still works (cache binaries for your own Bynder ingestion). See § *Bynder backend*.
...
- **`array-of-blocks`** (richtext) — converted to Portable Text via `@portabletext/block-tools` (parsed through `jsdom`). Decorators (`strong`, `em`, `underline`, `strike-through`, `code`), styles (`normal`, `h1`–`h4`, `blockquote`), lists (`bullet`, `number`), and `link` annotations are preserved. HTML `<table>` elements become Sanity's native Portable Text table blocks (`table → rows[] → cells[] → value`, Studio ≥ 6.6): `headerRows` comes from `<thead>` (or leading all-`<th>` rows), cell content goes through the same HTML pass so marks/links/lists survive, `colspan`/`rowspan` are dropped (content kept, rows padded rectangular), `<caption>` content is preserved as text block(s) before the table, nested tables flatten to text inside the parent cell, and malformed tables fall back to plain flattened blocks. Keys are derived from a SHA1 of `{jcrPath}::{field}:{counter}` so re-runs produce byte-identical clean docs.
...
- **`date` / `datetime`** — AEM datepickers persist whatever their `valueFormat` says (ISO-8601-with-offset when unset, display-style strings like `"May 23, 2024"` when set). The registry records the format; transform parses with it and re-emits `YYYY-MM-DD` (dates keep the literal date part — no timezone shifting) or UTC ISO (datetimes). ISO inputs and the common `MMM DD, YYYY` shape coerce even without a recorded format; anything unparseable keeps the original.
...
Before coercing, authored keys that differ from a declared field only by letter case are renamed onto the declared name (`linkURL` → `linkUrl`, `altValueFromDAM` → `altValueFromDam`) — the schema emitter camelCases dialog `name`s while the JCR persists them raw, and without the rename the value would surface as an "Unknown field" next to an empty declared one. Only case-only mismatches rename; genuinely different keys still pass through and show up in the drift report.
...
`flatten: true` (optional) drops the container's own block and hoists its items into the parent's pageBuilder array — for pure-layout containers like AEM's responsive grid. Exception: a flatten container carrying `cq:panelTitle` is a **tabs / accordion panel** (AEM drops the same container type into panels and stamps the title on it) — it keeps its block, with the title lifted to `panelTitle` via the authoring-hints opt-in for that resource type. Register the tabs/accordion component itself as a plain non-flatten container. `document: true` (optional, mutually exclusive with `flatten`) extracts every instance into its own `contentFragment` document with a `contentFragmentRef` block in the parent array — the right model for recursive structures (tabs, accordions): depth resets per document, one consistent Studio/query shape, one GROQ join per configured type. Reported under `transform-report.json → configExtractedFragments`. Frontends resolve the ref with `fragment->{ _id, title, content }` wherever they project a page-builder array — query patterns (including nested-fragment strategies) are documented in `docs/aem-to-sanity-mapping.md` § "Container components" and `docs/running-the-migration.md` § 1c-quater.

When a page's preserved panels would push it past Sanity's hard 20-level attribute-depth limit, the transform cuts the offending subtree into a standalone `contentFragment` document (referenced from the original position via a `contentFragmentRef` block, imported in the same per-page transaction) — lossless, listed in `transform-report.json → depthExtractedFragments`. Shapes that can't be cut fall back to flattening the deepest titled panel (`depthFlattenedPanels`). **Styled wrappers** (a dialog full of per-breakpoint layout config — padding, gap, height, background for mobile/tablet/desktop — around a drop zone) are just non-flatten containers: their layout fields survive with full coercion and their children land in `childrenField`, whether authored directly on the wrapper node or inside a nested responsive-grid container (a nested `flatten: true` container collapses into the wrapper's items; the wrapper's own block stays). A childless wrapper emits an empty array.
...
The array-vs-single decision is made once by the schema (from its global view of every page) and recorded in `content-type-registry.json`; the transform reads it back so both sides agree on the shape regardless of how many instances a given page carries. There's no config for named-slot *shape* — it's inferred from content. Container parents (listed in `aem-component-containers.json`) skip slot synthesis so their drop-zone children stay on the container-items path. The optional `aem-component-slots.json` adds schema-side visibility rules (`visibleWhen`) that fold a slot field behind its AEM enable-toggle in the Studio; the transform ignores that file — hidden or not, authored slot content always migrates.
...
Runs phase 1 for real (real AEM download, no ML dedup lookup beforehand) and stops — phases 0, 2, 3, 4 never run, and none of `SANITY_MEDIA_LIBRARY_ID` / `SANITY_TOKEN` / `SANITY_PROJECT_ID` / `SANITY_ML_LINK_TOKEN` need to be set. Unlike plain dry-run (which also stops after phase 1 but is meant as a preview before a real ML run), `--download-only` is an intentional terminal stage: the resulting `output/cache/assets/*` binaries and `manifest.json` (damPath → cachedFile → mimeType → fileSize) are a stable handoff point for a future non-Media-Library uploader — e.g. a Bynder integration that reads the manifest, uploads each `cachedFile`, and performs its own doc rewrite. Works the same whether `MIGRATION_DRY_RUN` is `true` or `false`. Mutually exclusive with `--link-only` and `--upload-only`; composable with `--placeholders` / `AEM_FIXTURES_DIR` (they only change where phase 1 sources binaries from).

### Bynder backend (`MIGRATION_ASSET_BACKEND=bynder`)

For clients whose assets were migrated into a [Bynder](https://www.bynder.com/) portal out-of-band. Contract: every migrated asset carries a Bynder **metaproperty** (name of your choosing — `BYNDER_AEM_PATH_PROPERTY`) whose value is the asset's legacy AEM DAM path. `aem-assets` never uploads to Bynder.

Phase 0 becomes Bynder resolution (runs under dry-run too — read-only preview): each unique DAM path is looked up via `GET {BYNDER_BASE_URL}/api/v4/media/?property_{name}={path}` with an exact (trimmed) client-side match on the echoed metaproperty value, falling back to a `keyword` search when the server-side filter returns nothing; multiple exact matches resolve to the most recently modified asset (logged). Phases 1–3 are skipped. Phase 4 rewrites each field to the value shape [`sanity-plugin-bynder-input`](https://github.com/sanity-io/plugins/tree/main/plugins/sanity-plugin-bynder-input) persists (`{_type:'bynder.asset', id, databaseId, name, type:'IMAGE', previewUrl, previewImg, datUrl, videoUrl, width, height, aspectRatio}`) — `migrate:schema` emits the matching `bynder.asset` field types, and the consuming Studio activates the plugin via the flag-gated `aemBynderPlugin()` helper from `aem-to-sanity-studio` (set `SANITY_STUDIO_BYNDER_PORTAL_URL` in the Studio `.env`). Manifest entries carry `bynderAssetId` + the `bynder.asset` object in `sanityRef`; misses surface exactly like ML link failures (per-path `miss` lines, `assets-report.json → rewrite.unresolved`, `assets-failures.log`). Every run re-resolves every path, so metaproperty fixes in Bynder are picked up without local state surgery. A startup connection probe fails fast on a bad `BYNDER_BASE_URL`/`BYNDER_TOKEN`, and a best-effort taxonomy check warns when `BYNDER_AEM_PATH_PROPERTY` names no metaproperty. **Set-once knob** shared with `migrate:schema` — switching backends after an import orphans ingested field values. Full walkthrough: `docs/running-the-migration.md` § 4c-sexies.
