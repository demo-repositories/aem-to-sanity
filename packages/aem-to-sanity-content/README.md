...
`flatten: true` (optional) drops the container's own block and hoists its items into the parent's pageBuilder array — for pure-layout containers like AEM's responsive grid. Exception: a flatten container carrying `cq:panelTitle` is a **tabs / accordion panel** (AEM drops the same container type into panels and stamps the title on it) — it keeps its block, with the title lifted to `panelTitle` via the authoring-hints opt-in for that resource type. Register the tabs/accordion component itself as a plain non-flatten container. `document: true` (optional, mutually exclusive with `flatten`) extracts every instance into its own `contentFragment` document with a `contentFragmentRef` block in the parent array — the right model for recursive structures (tabs, accordions): depth resets per document, one consistent Studio/query shape, one GROQ join per configured type. Reported under `transform-report.json → configExtractedFragments`.
