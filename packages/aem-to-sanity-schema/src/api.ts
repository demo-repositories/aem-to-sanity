...
import { mapDialog, type NodeFetcher } from "./mapper.ts";
...
import { writePageBuilderArtifacts } from "./pagebuilder.ts";
...
export async function migrateSchemas(
  ...
  if (emitPageBuilder) {
    const componentTypeNames = report.results
  ...
  await writeSchemasBarrel(outputDir, report, { emitPageBuilder });
...
async function processOne(
  ...
  report.add({
    ...
    outputFile,
    fieldNames: mapped.fields.map((f) => f.name),
