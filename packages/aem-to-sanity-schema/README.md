...
**Dialog inheritance via `sling:resourceSuperType` (Sling Resource Merger).** The migrator walks the **entire** `sling:resourceSuperType` chain (across `/apps` then `/libs`, AEM's lookup order), collects every `cq:dialog` found along it — including the component's own — and merges them with Sling Resource Merger semantics, the more-derived dialog overlaying its ancestors' (child wins property collisions, same-named nodes merge recursively, inherited children order first with child-only nodes appended, `sling:orderBefore` / `sling:hideProperties` / `sling:hideChildren` / `sling:hideResource` honored and then stripped). This is what AEM does at request time through `/mnt/override` — a proxy or versioned component whose dialog only declares additions on top of an ancestor (e.g. `title` adding Display/Styles tabs over Core Components' Properties tab) migrates with the inherited tabs intact. A failure past the first dialog found degrades to a logged warning instead of failing the component. Chains are recorded in `migration-report.json` under each component's `supertypeChain`, with `contributingPaths` listing which entries supplied a dialog (more than one = merged), and logged at info level during the run. Registry keys remain the original proxy path's resource type — so authored content with `sling:resourceType: <proxy>` keeps matching its emitted Sanity type even though dialog fields came from ancestors.
